...
export class AuthorServiceImpl implements AuthorService {
  async getAuthor(id: number): Promise<Author | undefined> {
    const response = await fetch(`${API_URL}/v1/authors/${id}`, {
      method: "GET",
      headers: {
  ...
  async getAuthors(): Promise<Author[]> {
    const response = await fetch(`${API_URL}/v1/authors`, {
      method: "GET",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
  ...
  async fullUpdateAuthor(id: number, author: Author): Promise<Author> {
    const response = await fetch(`${API_URL}/v1/authors/${id}`, {
      method: "PUT",
      headers: {
        Accept: "application/json",
  ...
  async partialUpdateAuthor(id: number, author: AuthorUpdateRequest): Promise<Author> {
    const response = await fetch(`${API_URL}/v1/authors/${id}`, {
      method: "PATCH",
      headers: {
        Accept: "application/json",
